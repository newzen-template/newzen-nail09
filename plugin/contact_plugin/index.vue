<template>
  <div v-if="block.status" :data-cms-bind="dataBinding" :style="{
    ...
    <div class="container relative">
      <div class="text-center w-full">
        <h2 v-if="block.title" class="custom_title_plugin" v-html="block.title"></h2>
        <div v-if="block.sub_title" v-html="block.sub_title" class="custom_subtitle"></div>
      </div>
      <div class="w-full flex flex-col justify-center items-center">
        <div :class="{
          ...
          <div v-if="block.form" class="grid w-full md:grid-cols-2 grid-cols-1 gap-x-[30px]">
            <div v-for="(item, key) in block.form" :key="key" :class="{
              'col-span-1': item.width_fields === '1/2',
              'md:col-span-2': item.width_fields === 'full',
            }">
              <div class="flex flex-col gap-[15px] justify-start items-start w-full mt-[15px]"
                v-if="item.fields === 'text'">
                <div class="w-full flex flex-col relative">
                  <label v-if="item.label" :style="`color: ${block.color_text}`" class="custom_label">{{
                    item.label }}<span class="text-red-500 ml-[3px] font-light text-[14px]"
                      v-if="item.status_error">*</span></label>
                  <input :type="item.field" v-model="formState[key].value"
                    ...
                    :placeholder="item.placehoder ? item.placehoder : ''" style="box-shadow: 0 0 0 transparent;" />
                  <div v-if="item.status_error && fieldError[key] && !formState[key].value">
                    <span class="text-red-500 text-[14px] font-medium">{{ item.error }}</span>
                  </div>
                </div>
              </div>
                ...
                <div class="w-full flex flex-col relative">
                  <label v-if="item.label" :style="`color: ${block.color_text}`" class="custom_label">{{
                    item.label }}<span class="text-red-500 ml-[3px] font-light text-[14px]"
                      v-if="item.status_error">*</span></label>
                  <input :type="item.field" v-model="formState[key].value"
                    :class="{ 'border-red-500': (item.status_error === true) && ((fieldError[key] === true && !formState[key].value) || errorEmail === true) }"
                    class=" outline-none text-[14px] h-[36px] w-full py-[6px] px-[12px] bg-white border" :style="{
                      borderRadius: block.border ? `${block.border}px` : '0px',
                      color: block.color_text,
                    }" :placeholder="item.placehoder ? item.placehoder : ''" style="box-shadow: 0 0 0 transparent;" />
                  <div v-if="item.status_error">
                    ...
                    <div v-else-if="errorEmail">
                      <span class="text-red-500 text-[14px] font-medium">Invalid email format.</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div class="flex flex-col gap-[15px] justify-start items-start w-full mt-[15px]"
                ...
                <div class="w-full flex flex-col relative">
                  <label v-if="item.label" :style="`color: ${block.color_text}`" class="custom_label">{{
                    item.label }}<span class="text-red-500 ml-[3px] font-light text-[14px]"
                      v-if="item.status_error">*</span></label>
                  <input :type="item.field" v-model="formState[key].value" @input="onPhoneNumberInput($event)"
                    maxlength="12"
                    :class="{ 'border-red-500': (item.status_error === true) && ((fieldError[key] === true && !formState[key].value) || errorPhone === true) }"
                    class=" outline-none text-[14px] h-[36px] w-full py-[6px] px-[12px] bg-white border" :style="{
                      borderRadius: block.border ? `${block.border}px` : '0px',
                      color: block.color_text,
                    }" :placeholder="item.placehoder ? item.placehoder : ''" style="box-shadow: 0 0 0 transparent;" />
                  <div v-if="item.status_error">
                    ...
                    <div v-else-if="errorPhone">
                      <span class="text-red-500 text-[14px] font-medium">Invalid phone number format.</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div class="flex flex-col gap-[15px] justify-start items-start w-full mt-[15px]"
                ...
                <div class="w-full flex flex-col relative">
                  <label v-if="item.label" :style="`color: ${block.color_text}`" class="custom_label">{{
                    item.label }}<span class="text-red-500 ml-[3px] font-light text-[14px]"
                      v-if="item.status_error">*</span></label>
                  <textarea rows="5" v-model="formState[key].value"
                    :class="{ 'border-red-500': (item.status_error && fieldError[key] && !formState[key].value) === true }"
                    class=" outline-none text-[14px] w-full py-[6px] px-[12px] bg-white border" :style="{
                      borderRadius: block.border ? `${block.border}px` : '0px',
                      color: block.color_text,
                    }" :placeholder="item.placehoder ? item.placehoder : ''"
                    style="box-shadow: 0 0 0 transparent;"></textarea>
                  <div v-if="item.status_error && fieldError[key] && !formState[key].value">
                    <span class="text-red-500 text-[14px] font-medium">{{ item.error }}</span>
                  </div>
                </div>
              ...
              <div class="flex flex-col justify-start items-start w-full mt-[15px]" v-else-if="item.fields === 'file'">
                <label v-if="item.label" :style="`color: ${block.color_text}`" class="custom_label">{{
                  item.label }}<span class="text-red-500 ml-[3px] font-light text-[14px]"
                    v-if="item.status_error">*</span></label>
                <div @dragover.prevent @drop.prevent="handleDropFiles($event, key)"
                  ...
                  <div class="grid gap-1">
                    ...
                    <span class="text-center text-gray-400 text-xl leading-4">{{ item.placehoder }}</span>
                  </div>
                      ...
                      <div class="flex items-center gap-2">
                        ...
                        <div class="grid gap-1">
                          <NuxtLink :to="file.url" target="_blank">
                            <span class="text-gray-900 text-sm font-normal font-['Inter'] leading-snug">{{ file.name
                              }}</span>
                          </NuxtLink>
                          <span class="text-gray-400   text-xs font-normal font-['Inter'] leading-[18px]">Upload
                            complete
                          </span>
                        </div>
                      </div>
                      <div class=" flex items-center gap-2">
                        <div class="relative">
                          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                            <path fill="none" stroke="#1ba300" stroke-linecap="round" stroke-linejoin="round"
                              stroke-width="1.5" d="M9 12.75L11.25 15L15 9.75M21 12a9 9 0 1 1-18 0a9 9 0 0 1 18 0" />
                          </svg>
                        </div>
                        <div @click="RemoveFile(index, key, file.url)" class="icon-cancel cursor-pointer ">
                          <svg class="" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                            fill="none">
                            <g id="Upload 3">
                              <path class=" stroke-2 stroke-red-500 " id="icon"
                                ...
                                stroke="#D1D5DB" stroke-width="1.6" stroke-linecap="round" />
                            </g>
                          </svg>
                        </div>
                      </div>
                      ...
                      <div class="flex items-center gap-2">
                        ...
                        <div class="grid gap-1">
                          ...
                          <span v-if="!statusUploadFile[key]?.value"
                            class="text-red-400 text-xs font-bold font-['Inter'] leading-[18px]">Upload Fail
                          </span>
                          <span v-else class="text-gray-400 text-xs font-normal font-['Inter'] leading-[18px]">Inprocess
                          </span>
                        </div>
                      </div>
                      <div class=" flex items-center gap-2">
                        ...
                        <span v-if="statusUploadFile[key]?.value"
                          class="ml-2 bg-white rounded-full  text-gray-800 text-xs font-medium flex justify-center items-center ">{{
                            percentCompleteUploadFile[key].value
                              ? `${percentCompleteUploadFile[key].value}%` : '0%' }}</span>
                      </div>
                    </div>
                  </div>
                </div>
    ...
    <div class="popup-notification" v-if="showNotification" :class="{ active: showNotification === true }">
      <div class="popup-content relative p-4">
        <div class="absolute top-2 right-2 w-[20px] h-[20px] cursor-pointer" @click="closePopup()">
...
function ShowPopup(title: "Success" | "Error" | "Warning", message: string, color: "success" | "error" | "warning", active: boolean) {
...
const handleOnchaneTypeFile = (event: any, key: any) => {
  ...
  const totalSizeFiles = sizeFiles.reduce((accumulator, currentValue) => accumulator + currentValue)
  ...
  if (totalSizeFiles > maxBytes) {
    ShowPopup('Error', 'PNG, JPG or PDF, smaller than 15MB dasds', 'error', true)
    return
  }
  const file = {}
  if (inputFile) {
...
function UpdateFilesComplete(listFile: any, key: number) {
  listFilesComplete[key] = {
    value: listFilesComplete[key]?.value ? [...listFilesComplete[key]?.value, listFile].flat() : listFile
  }
}
...
function UpdateFormstateForFile(listUrl: any, key: number) {
  ...
  const fileValue = { ...formState[key], value: listUrl.length > 0 ? values : '' }
...
function UpdateListFiles(value: any[], key: any) {
  ...
  Object.entries(formTest).forEach(([key, value]) => formData.append(`${key}`, value))
...
async function RemoveFile(index: number, key: any, url: string) {
  try {
    ...
    ShowPopup('Success', 'File has been successfully deleted!', 'success', true)
  ...
  for (const key in formState) {
    if (formState.hasOwnProperty(key)) {
      ...
      if (item.type === 'tel') {
        ...
        //   input = input.slice(1);
        // }
        const filteredInput = input.split('').filter(char => char >= '0' && char <= '9').join('');
        let formattedInput;
        if (filteredInput.length > 6) {
          formattedInput = filteredInput.replace(/(\d{3})(\d{3})(\d{1,4})/, '$1-$2-$3');
    ...
    if (formState.hasOwnProperty(key)) {
      ...
      if (item.status_error) {
        if (typeof item.value !== 'object' && !item.value.trim()) {
          ...
          } else if (item.type === 'email' && !emailRegex.test(item.value.trim())) {
            fieldError[key] = true;
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.onload = function () {
      ...
      if (xhr.status === 200) {
        ...
        ShowPopup('Error', 'Only files with these MIME types are allowed: image`/*`, application/pdf.', 'error', true)
    ...
    xhr.onerror = function () {
      ...
      // UpdateFormstateForFile([], key)
      ShowPopup('Error', 'An error occurred during file upload. Please check the file format or size and try again. If the issue persists, contact support for assistance.', 'error', true)
    };
...
const onSubmit = async () => {
  try {
    ...
    await $fetch('https://contact-form-api.nz-service01.dtsmart.dev/api/v1/form-submission', {
      method: 'POST',
      body: {
        ...body
      }
